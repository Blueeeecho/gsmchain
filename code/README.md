...
本文档说明如何使用 `eval_chaingsm.py` 对 ChainGSM 数据进行本地模型评测。脚本使用 `vLLM` 加速推理，默认在 `math-noise` 虚拟环境中运行。
...
```bash
conda run --no-capture-output -n math-noise python /home/wwq416/snap/wwq/math-chain/code/eval_chaingsm.py
```
...
- 如果使用普通 `conda run -n math-noise ...`，conda 可能会捕获输出，导致 tqdm 进度条不实时显示。建议使用 `conda run --no-capture-output -n math-noise ...`。
