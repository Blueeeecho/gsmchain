...
def main() -> None:
    parser = config_arg_parser("Train ChainGSM DPO with TRL.")
    ...
    print(f"DPO run dir: {run_dir}", flush=True)
    print(f"DPO model: {model_path}", flush=True)
    print(f"DPO train file: {train_file}", flush=True)
    print(f"DPO train examples: {len(rows)}", flush=True)
    ...
    dpo_args = DPOConfig(**accepted_kwargs(DPOConfig.__init__, training_cfg))
    ...
    trainer = DPOTrainer(
        model=model_path,
        ref_model=config["model"].get("ref_model_name_or_path"),
        args=dpo_args,
    ...
    print(f"DPO finished. Output: {run_dir}")
