...
def reward_func(completions, reward_reference=None, **kwargs):
    references = reward_reference or kwargs.get("references") or [{} for _ in completions]
    rewards = []
    for completion, reference in zip(completions, references):
        reward, _ = score_response(completion, reference or {}, **kwargs)
        rewards.append(reward)
    return rewards


def compute_reward(data_source=None, solution_str=None, ground_truth=None, extra_info=None, **kwargs):
