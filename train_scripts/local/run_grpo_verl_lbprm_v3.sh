...
#
# 关键设计:
#   - SFT 起点固定为 sft_2epoch/20260531_152306/checkpoints/best(26.59% on 5467)
#   - 步数硬上限 400,SAVE_FREQ=100(每 100 step 触发评测)
#   - reward = reward_chaingsm_lbprm_v3_verl.py
#   - 测试集 5467 条
#
# 终止条件:
#   1) 达到 400 step 自动停
#   2) 评测"无提升"连续 2 次(200 步之后)则停(本脚本不强制,只跑 400)
#
# 输出:
#   outputs/train/local/grpo_verl_lbprm_v3/Qwen2.5-0.5B-Instruct/<RUN_NAME>/<RUN_ID>/
#
# 用法:
#   RUN_NAME=grpo_verl_v3_400step \
#   bash train_scripts/local/run_grpo_verl_lbprm_v3.sh 2>&1 | tee /tmp/v3_run.log
...
RUN_NAME="${RUN_NAME:-grpo_verl_v3}"
RUN_ID="${RUN_ID:-$(date +%Y%m%d_%H%M%S)}"
OUTPUT_DIR="${OUTPUT_DIR:-${ROOT}/outputs/train/local/grpo_verl_lbprm_v3/Qwen2.5-0.5B-Instruct/${RUN_NAME}/${RUN_ID}}"
...
"$PYTHON" -m verl.trainer.main_ppo \
  ...
  trainer.project_name=chaingsm_lbprm_v3 \
