...
#
# 关键设计:
#   - 与 v3 唯一区别: 训练 parquet 换为 verl_grpo_train_neutral.parquet (NEUTRAL prompt)
#   - SFT 起点固定为 sft_2epoch/20260531_152306/checkpoints/best
#   - 步数硬上限 400, SAVE_FREQ=100 (每 100 step 触发评测)
#   - reward = reward_chaingsm_lbprm_v3_verl.py (沿用 v3)
#   - 测试集 5467 条
#
# 终止条件:
#   1) 达到 400 step 自动停
#
# 输出:
#   outputs/train/local/grpo_verl_lbprm_v4/Qwen2.5-0.5B-Instruct/<RUN_NAME>/<RUN_ID>/
#
# 用法:
#   RUN_NAME=grpo_verl_v4 \
#   bash train_scripts/local/run_grpo_verl_lbprm_v4.sh 2>&1 | tee /tmp/v4_run.log
...
REWARD_PATH="${REWARD_PATH:-${ROOT}/train_pipeline/reward_chaingsm_lbprm_v3_verl.py}"

MAX_STEPS="${MAX_STEPS:-400}"
SAVE_FREQ="${SAVE_FREQ:-100}"

RUN_NAME="${RUN_NAME:-grpo_verl_v4}"
RUN_ID="${RUN_ID:-$(date +%Y%m%d_%H%M%S)}"
OUTPUT_DIR="${OUTPUT_DIR:-${ROOT}/outputs/train/local/grpo_verl_lbprm_v4/Qwen2.5-0.5B-Instruct/${RUN_NAME}/${RUN_ID}}"
...
echo "=========================================="
echo "LB-PRM v4 GRPO 训练 (NEUTRAL prompt 对齐)"
echo "=========================================="
...
echo "  MODEL:          $MODEL"
echo "  DATA:           $DATA"
echo "  REWARD:         $REWARD_PATH (v3 reward 沿用)"
...
if [[ "$EVAL_ENABLED" == "1" && "$EVAL_BASELINE" == "1" ]]; then
  echo ""
  echo "=== Baseline evaluation (sft_2epoch/best) ==="
...
"$PYTHON" -m verl.trainer.main_ppo \
  ...
  trainer.project_name=chaingsm_lbprm_v4 \
