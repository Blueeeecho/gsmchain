...
#   - REWARD: reward_chaingsm_lbprm_v7_verl.py (新建, 8-shot CoT 协议适配)
...
#
# 输出:
#   outputs/train/local/grpo_verl_lbprm_v7/Qwen2.5-0.5B-Instruct/<RUN_NAME>/<RUN_ID>/
#
# 用法:
#   RUN_NAME=grpo_verl_v7 \
#   bash train_scripts/local/run_grpo_verl_lbprm_v7.sh 2>&1 | tee /tmp/v6_run.log
...
# v7 起点: 0.5B base (无 SFT, 干净起点)
# 0.5B base + 8-shot CoT 原生 original = 0.4329 (实测, 1319 条)
# sft_2epoch/best + 8-shot CoT original = 0.1296 (实测, SFT 摧毁 8-shot 能力)
...
OUTPUT_DIR="${OUTPUT_DIR:-${ROOT}/outputs/train/local/grpo_verl_lbprm_v7/Qwen2.5-0.5B-Instruct/${RUN_NAME}/${RUN_ID}}"
...
# v7 reward 权重 (3 子项: format / answer / numeric_correctness)
# answer 0.70 (主线目标: original 数字对)
# numeric_correctness 0.20 (0.5B base 算式正确率 22% 是薄弱点)
# format 0.10 (base 0.5B 自由推理下基本 ok)
FORMAT_WEIGHT="${FORMAT_WEIGHT:-0.10}"
ANSWER_WEIGHT="${ANSWER_WEIGHT:-0.70}"
NUMERIC_CORRECTNESS_WEIGHT="${NUMERIC_CORRECTNESS_WEIGHT:-0.20}"
...
echo "=========================================="
echo "LB-PRM v7 GRPO 训练 (8-shot CoT 协议)"
echo "=========================================="
...
echo "  NUMERIC_CORRECTNESS: $NUMERIC_CORRECTNESS_WEIGHT"
...
"$PYTHON" -m verl.trainer.main_ppo \
  ...
  trainer.project_name=chaingsm_lbprm_v7 \
