...
MODEL="${MODEL:-/home/wwq416/snap/wwq/math-chain/outputs/train/local/sft/Qwen2.5-0.5B-Instruct/sft_cot_2ep_resume/20260617_180315/checkpoints/checkpoint-762}"

# Data path
DATA="${DATA:-${ROOT}/chaingsm_data/data/final/grpo/all_grpo_cot.parquet}"

# Reward function path
REWARD_PATH="${REWARD_PATH:-${ROOT}/train_pipeline/reward_chaingsm_v10_verl.py}"

# Run identification
RUN_NAME="${RUN_NAME:-grpo_v10_signed}"
...
if [[ "$PROFILE" == "paper" ]]; then
  ...
  DEFAULT_ROLLOUT_N=4
  DEFAULT_ROLLOUT_GPU_MEM_UTIL=0.5
...
TOTAL_GRPO_STEPS="${TOTAL_GRPO_STEPS:-0}"  # 0 = use total_epochs × steps_per_epoch (don't pass to verl)
if [ "$TOTAL_GRPO_STEPS" = "0" ]; then
  TOTAL_GRPO_STEPS_ARG=""
else
  TOTAL_GRPO_STEPS_ARG="trainer.total_training_steps=$TOTAL_GRPO_STEPS"
fi
...
EXPRESSION_WEIGHT="${EXPRESSION_WEIGHT:-1.2}"
TRACE_WEIGHT="${TRACE_WEIGHT:-0.7}"
...
EVAL_GPU_MEM_UTIL="${EVAL_GPU_MEM_UTIL:-0.25}"
EVAL_BATCH_SIZE="${EVAL_BATCH_SIZE:-64}"
EVAL_BASELINE="${EVAL_BASELINE:-0}"  # Run baseline eval before training
...
if [ "$TOTAL_GRPO_STEPS" = "0" ]; then
  ...
  FULL_TRAINING_STEPS=$TOTAL_GRPO_STEPS
fi
...
echo "  Eval data:     $EVAL_DATA_PATH (method=cot_brackets)"
echo "  Total GRPO steps: $TOTAL_GRPO_STEPS"
echo "  Logs:          $LOG_DIR"
...
for EPOCH_NUM in $(seq 1 $TOTAL_EPOCHS); do
  ...
  if ! "$PYTHON" -m verl.trainer.main_ppo \
    ...
    +custom_reward_function.reward_kwargs.core_weight=1.2 \
    +custom_reward_function.reward_kwargs.core_trace_w=0.7 \
    +custom_reward_function.reward_kwargs.core_final_w=0.3 \
    +custom_reward_function.reward_kwargs.calc_weight=0.3 \
    +custom_reward_function.reward_kwargs.calc_step_w=0.8 \
    +custom_reward_function.reward_kwargs.calc_final_w=0.2 \
