#!/usr/bin/env bash
# verl GRPO v12i training: V12 prompt + V12i reward (4 项, 跟 V12 公式同构)
# 起点: Qwen2.5-0.5B-Instruct base (跳过 SFT, V12i 重新 GRPO)
# 训练量: 2 epoch × 762 steps/epoch = 1524 total steps
# 保存: 每 300 步
# Reward: 4 项 (3.0*answer + 1.5*step_value_lcs + 0.5*step_value_exact - 0.5*distractor_per_step)
...
# V12 数据 (3051 行) — V12i 沿用 V12 prompt + 数据, 只换 reward
DATA="${ROOT}/chaingsm_data/data/final/grpo/grpo_v12_json.parquet"

# V12i 奖励
REWARD_PATH="${ROOT}/train_pipeline/reward_chaingsm_v12i_json_verl.py"

# Run
RUN_NAME="${RUN_NAME:-qwen2.5-0.5b-grpo-verl-v12i-reward}"
...
# V12 prompt 4873 tokens, max_prompt_length 2048
...
# Snapshot configs
cp "${ROOT}/train_configs/local/grpo_verl_v12i.yaml" "$CONFIG_DIR/grpo_verl_v12i.yaml"
...
echo "[v12i] samples=$NUM_SAMPLES, batch=$TRAIN_BATCH_SIZE, steps_per_epoch=$STEPS_PER_EPOCH, total_epochs=$TOTAL_EPOCHS, total_steps=$TOTAL_STEPS"
...
cd "$ROOT"
echo "=========================================="
echo "verl GRPO v12i (V12 prompt + V12i reward, 2 epoch)"
echo "=========================================="
...
echo "  Max prompt:    $MAX_PROMPT_LENGTH (V12 prompt 4873, 2048 截断)"
echo "  Max resp:      $MAX_RESPONSE_LENGTH"
echo "=========================================="

# V12i 奖励: 4 项, 跟 V12 公式同构
"$PYTHON" -u -m verl.trainer.main_ppo \
  ...
  +custom_reward_function.reward_kwargs.step_value_exact_weight=0.5 \
