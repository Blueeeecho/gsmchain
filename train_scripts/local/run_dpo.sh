...
CONFIG="${CONFIG:-${ROOT}/train_configs/local/dpo.yaml}"
MODEL="${MODEL:-/home/wwq416/snap/wwq/model/Qwen/Qwen2.5-0.5B-Instruct}"
DATA="${DATA:-${ROOT}/chaingsm_data/data/final/rl_preprocessed/gsm8k_train_balanced_one_variant_14946/dpo_train.jsonl}"
RUN_NAME="${RUN_NAME:-dpo}"
RUN_ID="${RUN_ID:-$(date +%Y%m%d_%H%M%S)}"
OUTPUT_BASE_DIR="${OUTPUT_DIR:-${ROOT}/outputs/train/local/dpo/Qwen2.5-0.5B-Instruct/${RUN_NAME}}"
...
cd "$ROOT"
echo "DPO output dir: $OUTPUT_DIR"
"$PYTHON" -m train_pipeline.train_dpo_trl "${ARGS[@]}" "$@"
