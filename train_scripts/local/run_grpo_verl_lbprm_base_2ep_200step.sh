...
MODEL="${MODEL:-/home/wwq416/.cache/huggingface/hub/models--Qwen--Qwen2.5-0.5B-Instruct/snapshots/7ae557604adf67be50417f59c2c2f167def9a775}"
...
#   total = format_weight * format + answer_weight * answer + liveness_weight * chain_quality_score
# v2 ignores expression_weight/trace_weight/distractor_penalty kwargs (absorbed silently).
...
for EPOCH_NUM in $(seq 1 $TOTAL_EPOCHS); do
  ...
  # Discover every checkpoint produced by this epoch run and evaluate any
  # whose step counter is a positive multiple of $SAVE_FREQ (which is 200 by
  # default). This gives us per-200-step evaluation snapshots without
  # restarting verl each time.
...
" 2>/dev/null || echo "0.0")
      ...
      echo "{\"stage\":\"step_${CKPT_STEP}\",\"epoch\":${CURRENT_EPOCH},\"global_step\":${CKPT_STEP},\"overall_accuracy\":${ACCURACY},\"original_accuracy\":${ORIGINAL_ACCURACY},\"best_accuracy\":${BEST_ACCURACY},\"eval_dir\":\"${EPOCH_EVAL_DIR}\",\"current_model_dir\":\"${HF_MODEL_DIR}\",\"eval_status\":\"ok\"}" >> "$EVAL_SUMMARY"

      if float_gt "$ACCURACY" "$BEST_ACCURACY"; then
        BEST_ACCURACY="$ACCURACY"
        BEST_CKPT="$HF_MODEL_DIR"
        echo "  *** New best model (overall=$ACCURACY) ***"
      fi
    else
      echo "  Evaluation FAILED at global_step=$CKPT_STEP"
      echo "{\"stage\":\"step_${CKPT_STEP}\",\"epoch\":${CURRENT_EPOCH},\"global_step\":${CKPT_STEP},\"overall_accuracy\":\"\",\"best_accuracy\":${BEST_ACCURACY},\"eval_dir\":\"${EPOCH_EVAL_DIR}\",\"current_model_dir\":\"${HF_MODEL_DIR}\",\"eval_status\":\"failed\"}" >> "$EVAL_SUMMARY"
    fi
  done
