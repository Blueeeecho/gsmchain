#!/usr/bin/env bash
# verl GRPO v14 training: 4 字段 JSON + 12 rules + prose-表达式 交错 reasoning
# 起点: Qwen2.5-0.5B-Instruct base (跳过 SFT, V14 重新 GRPO)
# 训练量: 3 epoch × 762 steps/epoch = 2286 total steps
# 保存: 每 300 步
# Reward: 4 项 (3.0*answer + 1.5*step_value + 0.5*path_alignment - 0.5*distractor_leak)
...
# V14 GRPO 数据 (3051 行)
DATA="${ROOT}/chaingsm_data/data/final/grpo/grpo_v14_reasoning.parquet"

# V14 奖励
REWARD_PATH="${ROOT}/train_pipeline/reward_chaingsm_v14_reasoning_verl.py"

# Run
RUN_NAME="${RUN_NAME:-qwen2.5-0.5b-grpo-verl-v14-reasoning}"
...
# V14 prompt 1940 tokens, max_prompt_length 2048
...
# Snapshot configs
cp "${ROOT}/train_configs/local/grpo_verl_v14.yaml" "$CONFIG_DIR/grpo_verl_v14.yaml"
...
echo "[v14] samples=$NUM_SAMPLES, batch=$TRAIN_BATCH_SIZE, steps_per_epoch=$STEPS_PER_EPOCH, total_epochs=$TOTAL_EPOCHS, total_steps=$TOTAL_STEPS"
...
cd "$ROOT"
echo "=========================================="
echo "verl GRPO v14 (4 字段 JSON, 3 epoch)"
echo "=========================================="
...
echo "  Max prompt:    $MAX_PROMPT_LENGTH (v14 prompt ~1940 + 2x question)"
echo "  Max resp:      $MAX_RESPONSE_LENGTH (砍 steps, response 短)"
echo "=========================================="

# V14 奖励: 4 项, 跟 V12 公式同构
"$PYTHON" -u -m verl.trainer.main_ppo \
  ...
  +custom_reward_function.reward_kwargs.path_alignment_weight=0.5 \
  +custom_reward_function.reward_kwargs.distractor_leak_weight=0.5 \
  \
