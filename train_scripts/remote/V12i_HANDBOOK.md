...
> **本资源包目标**: 让合作者在本机或远程 SLURM 集群跑 V12i GRPO 训练, 沿用 `submit_grpo_v12_pipeline.sh` (V12i 是默认) 一键提交, 无需手改 remote_env.sh

---

> **2026-06-21 简化**: 本份文档原本只讲 `submit_grpo_v12i_verl.sh` 一键提交器 (已删除).
> 合作者跑 V12i **只走** `train_scripts/remote/submit_grpo_v12_pipeline.sh` (默认 V12i),
> 详细 4 版本总览见 `V12_V14_HANDBOOK.md`, 简明改 prompt 流程见 `data_preprocess_README.md`.
...
# 2) 干跑自检
DRY_RUN=1 SKIP_PREFLIGHT=1 bash train_scripts/remote/submit_grpo_v12_pipeline.sh
...
**`submit_grpo_v12_pipeline.sh` 是 V12i 专用提交器**, 默认 reward / data / config / epoch 全是 V12i, **无需手改 remote_env.sh**. 详见 §5.
...
│       ├── submit_grpo_v{12,13,14}_pipeline.sh             <-- 老 V10 提交器 (保留, V12i 不走)
│       ├── submit_grpo_v12_pipeline.sh             <-- ★ v2 新增: V12i 专用一键提交器
│       ├── submit_sft_verl.sh
...
**远程**几乎不需要改路径: `submit_grpo_v12_pipeline.sh` 默认值用 `${REMOTE_ROOT}` 拼路径, 合作者只需要 `export REMOTE_ROOT=/path/to/repo` 即可.

---

## 5. 远程提交: V12i 一键提交器

本资源包**新增了** `train_scripts/remote/submit_grpo_v12_pipeline.sh` —— 为 V12i 量身定做, 默认就指向 V12i 的 reward / data / config / epoch. 合作者**不再需要手改 remote_env.sh**, 一行命令就能提交.
...
# 3) 跳过预检, 正式提交
SKIP_PREFLIGHT=1 bash train_scripts/remote/submit_grpo_v12_pipeline.sh
...
bash -n train_scripts/remote/submit_grpo_v12_pipeline.sh
```
...
- **远程默认指向 V12i**: v2 资源包新增 `submit_grpo_v12_pipeline.sh` 已经把 2 个变量默认指 V12i, 合作者无需手改
...
train_scripts/remote/submit_grpo_v12_pipeline.sh          ~125 行
