#!/bin/bash -l
# submit_grpo_v14_pipeline.sh
#
# ChainGSM V14 end-to-end pipeline (one sbatch):
#   1) data_preprocess_v14.sh   (raw + supplementary -> grpo_v14_reasoning.parquet)
#   2) V14 verl GRPO training  (uses grpo_v14_reasoning.parquet)
#
# V14 default epochs=3 (reasoning prompt is longer than v12/v13).
#
# Override:
#   REMOTE_ROOT=...              bash submit_grpo_v14_pipeline.sh
#   DRY_RUN=1                    bash submit_grpo_v14_pipeline.sh  (print only)
#   SKIP_PREPROCESS=1            bash submit_grpo_v14_pipeline.sh  (reuse existing parquet)
#   V14_TOTAL_EPOCHS=2           bash submit_grpo_v14_pipeline.sh  (override epochs)

#SBATCH -J chaingsm-grpo-v14-pipeline
#SBATCH -p gpu-all
#SBATCH --gres gpu:1
#SBATCH -c 8
#SBATCH --mem 32GB
...
CONFIG_NAME="grpo_verl_v14_vllm"
JOB_TAG="v14"
TOTAL_EPOCHS="${V14_TOTAL_EPOCHS:-3}"
...
echo "[v14-pipeline] Step 1/2: data preprocess (raw -> grpo_v14_reasoning.parquet)"
echo "[v14-pipeline] Step 2/2: V14 verl GRPO training (epochs=${TOTAL_EPOCHS}, config=${CONFIG_NAME})"

CMD_PRE="bash ${SCRIPT_DIR}/data_preprocess_v14.sh"
...
echo "[v14-pipeline] Preprocess command:"
echo "  ${CMD_PRE}"
echo "[v14-pipeline] Train command:"
echo "  ${CMD_TRAIN}"

if [[ "${DRY_RUN:-0}" == "1" ]]; then
  echo "[v14-pipeline] DRY_RUN=1, skipping actual run"
  exit 0
fi
...
echo "[v14-pipeline] Done. Ckpts: ${REMOTE_OUTPUT_DIR}/chaingsm-grpo-verl-${JOB_TAG}/checkpoints"
nvidia-smi
