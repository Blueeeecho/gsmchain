...
METHODS = ("direct", "zero_shot_cot", "eight_shot_cot", "eight_shot_cot_completion")
SAMPLING_CONFIG = {
    "temperature": 0.0,
    "top_p": 1.0,
    "max_tokens": 512,
...
def build_eight_shot_completion_prompt(question: str) -> str:
    pieces: list[str] = []
    for example_question, example_answer in EIGHT_SHOT_EXAMPLES:
        pieces.append(f"Q: {example_question}\nA: {example_answer}")
...
def build_prompt(tokenizer: Any, method: str, question: str) -> str:
    if method == "eight_shot_cot_completion":
        return build_eight_shot_completion_prompt(question)
